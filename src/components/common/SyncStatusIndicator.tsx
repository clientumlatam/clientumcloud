    ...
    >
      ...
      {!effectiveOnline ? (
        ...
        <button
          type="button"
          onClick={() => setIsDetailsOpen(!isDetailsOpen)}
          className="flex items-center gap-1.5 px-2 py-0.8 rounded-full bg-slate-800/80 hover:bg-slate-800 border border-slate-700/80 text-slate-300 hover:text-white text-xs font-medium transition-all cursor-pointer select-none"
        ...
        >
          ...
          {showText && (
            <span className="hidden sm:inline text-[10px] font-semibold text-slate-300">
              Sincronizado
            </span>
          )}
        </button>
      ...
      {isDetailsOpen && (
        <>
          ...
          />
          <div className="absolute left-0 sm:left-auto sm:right-0 top-full mt-2 w-72 sm:w-80 z-50 rounded-2xl border border-slate-700 bg-[#090F1E] p-4 text-slate-200 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150">
            {/* Popover Header */}
            ...
            <div className="py-3 space-y-2 text-xs">
              <div className="p-2.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-slate-400">Navegador (navigator.onLine):</span>
                  <span
                ...
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-slate-400">Recursos en Caché SW:</span>
                ...
                <div className="flex items-center justify-between text-[11px]">
                  ...
                  <span className="text-slate-300 font-mono">{lastSyncTime} hs</span>
                </div>
              </div>
            ...
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
              <span className="text-[10px] text-slate-400">Simulación de pruebas:</span>
              <button
                type="button"
