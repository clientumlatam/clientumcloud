      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-[var(--border-subtle)] dark:border-slate-800">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400 dark:text-[var(--text-muted)]">admin_console_dashboard.md</span>
          </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 rounded-xl bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] space-y-1 shadow-xs">
              <div className="text-[11px] text-[var(--text-muted)] dark:text-slate-400 font-medium">Usuarios Activos Mensuales</div>
            ...
            <div className="p-4 rounded-xl bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] space-y-1 shadow-xs">
              <div className="text-[11px] text-[var(--text-muted)] dark:text-slate-400 font-medium">Llamadas a Gemini 2.5 Flash</div>
            ...
            <div className="p-4 rounded-xl bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] space-y-1 shadow-xs">
              <div className="text-[11px] text-[var(--text-muted)] dark:text-slate-400 font-medium">Disponibilidad del Sistema</div>
              <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">99.98%</div>
              <div className="text-[10px] text-[var(--text-muted)] dark:text-slate-400">Zero downtime reportado</div>
            </div>
            <div className="p-4 rounded-xl bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] space-y-1 shadow-xs">
              <div className="text-[11px] text-[var(--text-muted)] dark:text-slate-400 font-medium">Volumen Transaccional</div>
              ...
              <tbody className="divide-y divide-[var(--border-subtle)] dark:divide-slate-800 text-[var(--text-secondary)] dark:text-slate-300">
                {usersList.map((u) => (
                  <tr key={u.id} className="hover:bg-[var(--bg-muted)]/80 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="p-3 font-mono text-[var(--text-muted)] dark:text-slate-400">{u.email}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3 text-[var(--text-muted)] dark:text-slate-400">{u.lastLogin}</td>
      ...
      {activeTab === 'logs' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-[var(--text-muted)] dark:text-slate-400">Inmutabilidad garantizada por SHA-256</span>
          </div>
              ...
              {auditLogs.map((log) => (
                <div key={log.id} className="p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 hover:bg-[var(--bg-muted)]/80 dark:hover:bg-slate-800/30 transition-colors">
                  <div className="flex items-center gap-3">
                    <span className="text-slate-400 dark:text-[var(--text-muted)] text-[11px]">{log.timestamp}</span>
                    <span className="text-indigo-600 dark:text-indigo-400 font-semibold">{log.user}</span>
                    <span className="text-[var(--text-primary)] dark:text-slate-200">{log.action}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-slate-400 dark:text-[var(--text-muted)] text-[11px]">{log.ip}</span>
                    <span
