      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500">vscrm_suite_dashboard.md</span>
          </div>
        ...
        {/* Live Timer Widget */}
        <div className="flex items-center gap-3 bg-slate-800/80 border border-slate-700/80 p-2.5 rounded-xl shadow-lg shrink-0">
          <div className="text-right">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Time Tracking Activo</div>
            <div className="text-lg font-mono font-bold text-emerald-400">{formatTimer(timerSeconds)}</div>
          </div>
          <button
            onClick={toggleTimer}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {projects.map((p) => {
              ...
              return (
                <div key={p.id} className="bg-slate-800/40 border border-slate-700/60 rounded-xl p-4 space-y-3 hover:border-slate-600 transition-all">
                  ...
                  <div className="pt-2 border-t border-slate-700/60 flex items-center justify-between text-xs">
                    ...
                    <span className="font-bold text-white font-mono">$ {p.budget.toLocaleString('es-AR')}</span>
                  </div>
                </div>
              );
            })}
              ...
              <tbody className="divide-y divide-slate-800 text-slate-300">
                {timeLogs.map((tl) => (
                  <tr key={tl.id} className="hover:bg-slate-800/30">
                    <td className="p-3 font-mono text-slate-400">{tl.date}</td>
                    <td className="p-3 font-semibold text-white">{tl.operator}</td>
                    <td className="p-3">
                      <div className="font-semibold text-slate-200">{tl.project}</div>
                      <div className="text-[11px] text-slate-400">{tl.task}</div>
                    </td>
                    <td className="p-3 text-right font-mono font-bold text-white">{tl.hours}h</td>
                    <td className="p-3 text-right font-mono text-slate-400">$ {tl.ratePerHour.toLocaleString('es-AR')}</td>
          ...
          <div className="p-4 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="p-3 rounded-lg bg-slate-800/70 border border-slate-700">
                <div className="text-xs font-bold text-white">Distribuidora del Sur S.A.</div>
                <div className="text-[11px] text-slate-400">Contrato: Retainer Mensual 40hs</div>
                <div className="text-[11px] text-emerald-400 mt-1">SLA: 4 horas de respuesta garantizada</div>
              </div>
              <div className="p-3 rounded-lg bg-slate-800/70 border border-slate-700">
                <div className="text-xs font-bold text-white">Logística & Envíos Express</div>
                <div className="text-[11px] text-slate-400">Contrato: Llave en Mano - Implementación</div>
                <div className="text-[11px] text-emerald-400 mt-1">SLA: Soporte 24/7 post lanzamiento</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="p-5 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-700">
              <div>
                <div className="text-xs font-bold text-white">Resumen de Horas Acumuladas Pendientes de Cobro</div>
                <div className="text-[11px] text-slate-400">Total acumulado durante el ciclo actual: 52.5 horas facturables.</div>
              </div>
              <button
                onClick={() => {
                ...
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-2"
              >
