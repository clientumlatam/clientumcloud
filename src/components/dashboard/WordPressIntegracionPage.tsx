      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500">wordpress_integracion_dashboard.md</span>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            disabled={isSyncing}
            className="px-3 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-5 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">URL del Sitio WordPress</label>
                <input
                  type="text"
                  value={siteUrl}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Consumer Key (ck_...)</label>
                <input
                  type="text"
                  value={consumerKey}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Consumer Secret (cs_...)</label>
                <input
                  type="password"
              ...
              <button
                ...
                className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-xs font-semibold transition-colors cursor-pointer flex items-center justify-center gap-2"
              >
          ...
          <div className="p-5 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-4">
            ...
            <div className="p-3 bg-slate-900 border border-slate-700 rounded-lg font-mono text-[11px] text-emerald-400 break-all select-all">
        ...
        <div className="space-y-6">
          <div className="p-5 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400">{syncedProducts.length} artículos sincronizados</span>
            </div>
                ...
                <tbody className="divide-y divide-slate-800 text-slate-300">
                  {syncedProducts.map((prod) => (
                    <tr key={prod.id} className="hover:bg-slate-800/30">
                      ...
                      <td className="p-3 text-right font-mono font-bold text-white">$ {prod.price.toLocaleString('es-AR')}</td>
                      <td className="p-3 text-right font-mono text-slate-300">{prod.stock} u.</td>
          ...
          <div className="p-5 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {orders.map((ord) => (
                <div key={ord.id} className="p-3 rounded-lg bg-slate-800 border border-slate-700 space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-white">{ord.id}</span>
                    <span className="text-emerald-400 font-bold font-mono">$ {ord.total.toLocaleString('es-AR')}</span>
                  </div>
                  <div className="text-xs text-slate-300">{ord.client}</div>
        ...
        <div className="p-5 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-slate-800/70 border border-slate-700 space-y-2">
              <div className="text-xs font-bold text-white">1. Confirmación de Compra</div>
              <p className="text-xs text-slate-400">
                Dispara mensaje con el resumen del pedido y link de seguimiento en cuanto el cliente abona en la tienda.
              </p>
              <span className="inline-block px-2 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
                Activo (Template Aprobado)
              </span>
            </div>

            <div className="p-4 rounded-xl bg-slate-800/70 border border-slate-700 space-y-2">
              <div className="text-xs font-bold text-white">2. Recupero de Carrito Abandonado</div>
              <p className="text-xs text-slate-400">
                A los 30 minutos de inactividad, envía recordatorio con cupón de 10% de descuento por WhatsApp.
              </p>
              <span className="inline-block px-2 py-0.5 rounded text-[10px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/30">
                Activo
              </span>
            </div>

            <div className="p-4 rounded-xl bg-slate-800/70 border border-slate-700 space-y-2">
              <div className="text-xs font-bold text-white">3. Notificación de Despacho</div>
              <p className="text-xs text-slate-400">
