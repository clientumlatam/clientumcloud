      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-[var(--border-subtle)] dark:border-slate-800">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400 dark:text-[var(--text-muted)]">wordpress_integracion_dashboard.md</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-5 bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] rounded-xl space-y-4 shadow-xs">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] dark:text-slate-400 mb-1">URL del Sitio WordPress</label>
                <input
                  type="text"
                  value={siteUrl}
                  onChange={(e) => setSiteUrl(e.target.value)}
                  className="w-full px-3 py-2 bg-[var(--bg-muted)] dark:bg-slate-900 border border-[var(--border-subtle)] dark:border-slate-700 rounded-lg text-xs text-[var(--text-primary)] dark:text-white focus:outline-none focus:border-indigo-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] dark:text-slate-400 mb-1">Consumer Key (ck_...)</label>
                <input
                  type="text"
                  value={consumerKey}
                  onChange={(e) => setConsumerKey(e.target.value)}
                  className="w-full px-3 py-2 bg-[var(--bg-muted)] dark:bg-slate-900 border border-[var(--border-subtle)] dark:border-slate-700 rounded-lg text-xs text-[var(--text-primary)] dark:text-white font-mono focus:outline-none focus:border-indigo-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] dark:text-slate-400 mb-1">Consumer Secret (cs_...)</label>
                <input
                  type="password"
          ...
          <div className="p-5 bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] rounded-xl space-y-4 shadow-xs">
            ...
            <div className="p-3 bg-[var(--bg-muted)] dark:bg-slate-900 border border-[var(--border-subtle)] dark:border-slate-700 rounded-lg font-mono text-[11px] text-emerald-700 dark:text-emerald-400 break-all select-all">
        ...
        <div className="space-y-6">
          <div className="p-5 bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] rounded-xl space-y-4 shadow-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-[var(--text-muted)] dark:text-slate-400">{syncedProducts.length} artículos sincronizados</span>
            </div>
                ...
                <tbody className="divide-y divide-[var(--border-subtle)] dark:divide-slate-800 text-[var(--text-secondary)] dark:text-slate-300">
                  {syncedProducts.map((prod) => (
                    <tr key={prod.id} className="hover:bg-[var(--bg-muted)]/80 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="p-3 font-mono text-[var(--text-muted)] dark:text-slate-400">{prod.sku}</td>
                      ...
                      <td className="p-3 text-right font-mono text-[var(--text-secondary)] dark:text-slate-300">{prod.stock} u.</td>
          ...
          <div className="p-5 bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] rounded-xl space-y-4 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {orders.map((ord) => (
                <div key={ord.id} className="p-3 rounded-lg bg-[var(--bg-muted)] dark:bg-slate-800 border border-[var(--border-subtle)] dark:border-slate-700 space-y-1">
                  ...
                  <div className="text-xs text-[var(--text-secondary)] dark:text-slate-300">{ord.client}</div>
      ...
      {activeTab === 'automation' && (
        <div className="p-5 bg-[var(--bg-card)] dark:bg-[#0D1527] border border-[var(--border-subtle)] dark:border-[#1E2E4A] rounded-xl space-y-4 shadow-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-[var(--bg-muted)] dark:bg-slate-800/70 border border-[var(--border-subtle)] dark:border-slate-700 space-y-2">
