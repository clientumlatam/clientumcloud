          ...
          <section className="min-w-0 rounded-xl border border-[#1e2330] bg-[#11141c]">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#1e2330] p-3">
              ...
              <div className="relative">
                ...
                <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar actividad" className="w-40 rounded-lg border border-[#2b3348] bg-[#0c1018] py-1.5 pl-8 pr-2 text-[11px] text-white outline-none placeholder:text-[var(--text-secondary)] focus:border-blue-500/60" />
              </div>
            </div>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        {isTask ? (
                          ...
                          <span className="rounded-md bg-[#1a2030] px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">{activity?.type === 'stage_change' ? 'Cambio de etapa' : activity?.type}</span>
                        )}
                        {item.target && <button onClick={() => openTarget(item)} className="flex items-center gap-1 px-1 py-1 text-[11px] font-semibold text-[var(--text-muted)] hover:text-slate-200">Ver registro <ChevronRight className="h-3 w-3" /></button>}
                      </div>
                    </div>
                  </article>
                );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <form onSubmit={submitActivity} className="w-full max-w-lg rounded-2xl border border-[#2a3348] bg-[#111722] p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between">
              ...
              <button type="button" onClick={() => setIsLogOpen(false)} className="rounded-md p-1 text-[var(--text-muted)] hover:bg-[#202838] hover:text-white"><X className="h-4 w-4" /></button>
            </div>
            ...
            <label className="mt-3 block text-[11px] font-semibold text-slate-300">Detalle
              <textarea required value={logText} onChange={(event) => setLogText(event.target.value)} rows={4} placeholder="¿Qué ocurrió y cuál es el próximo paso?" className="mt-1 w-full resize-none rounded-lg border border-[#2b3348] bg-[#0b1018] px-3 py-2 text-xs text-white outline-none placeholder:text-[var(--text-secondary)] focus:border-blue-500/60" />
            </label>
